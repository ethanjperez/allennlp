...
@DatasetReader.register("babi_triple")
class BabiTripleReader(DatasetReader):
    """
...
if __name__ == "__main__":
    ...
    babi_path = "datasets/babi_triple"
    ...
    for dt in ['train', 'valid', 'test']:
        dt_path = os.path.join(race_raw_path, "qa3_%s.txt" % dt)
        ...
        for i in range(len(lines)):
            ...
            if '\t' in lines[i]:
                ...
                # Get SPAN Sentence from sup_fact
                sup_fact = sup_fact.split()[-1]
    ...
    # Dump JSON Files
    with open(os.path.join(babi_path, "babi-triple-train-v1.0.json"), 'w') as f:
        json.dump(train_data, f)

    with open(os.path.join(babi_path, "babi-triple-dev-v1.0.json"), 'w') as f:
        json.dump(val_data, f)

    with open(os.path.join(babi_path, "babi-triple-test-v1.0.json"), 'w') as f:
