...
allennlp train training_config/race.best.debate.lr=2e-5.jsonnet -s tmp/race.a.m=sl-sents.bsz=64.lr=2e-5 -j tmp/race.best.f/model.tar.gz -b 1 -d a -m sl-sents -p tmp/race.best.f/oracle_outputs.all.pkl -a 64 -f
...
allennlp train training_config/race.best.debate.lr=2e-5.jsonnet -s tmp/race.b.m=sl-sents.bsz=64.lr=2e-5 -j tmp/race.best.f/model.tar.gz -b 1 -d b -m sl-sents -p tmp/race.best.f/oracle_outputs.all.pkl -a 64 -f
