...
export COMMAND="allennlp train training_config/race.best.debate.lr=1e-5.jsonnet -s tmp/race.bab.m=sl.bsz=32.lr=1e-5.c=concat -j tmp/race.best.f/model.tar.gz -b 1 -d bab -m sl -p tmp/race.best.f/oracle_outputs.c=concat.d=B_A_B_A_B_A_B_A.all.pkl -a 32 -c concat -f"
